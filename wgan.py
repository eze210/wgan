...
def create_G(Z_SIZE=Z_SIZE):
    ...
    x = Conv2DTranspose(256, (5, 5), strides=(2, 2), padding='same', kernel_initializer=weight_init)(x)
    #x = UpSampling2D(size=(2, 2))(x)
    #x = Conv2D(256, (5, 5), padding='same', kernel_initializer=weight_init)(x)
    x = LeakyReLU()(x)

    x = Conv2DTranspose(128, (5, 5), strides=(2, 2), padding='same', kernel_initializer=weight_init)(x)
    #x = UpSampling2D(size=(2, 2))(x)
...
def update_tb_summary(step, sample_images=True, save_image_files=True):
    ...
    if sample_images:
        ...
        arrayToReshape = [1] + list(img.shape) + [1]
        s.MergeFromString(tf.Session().run(tf.summary.image('samples_%07d' % step, img.reshape(arrayToReshape))))
